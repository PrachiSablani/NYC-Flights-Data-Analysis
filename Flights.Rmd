...
# 7. 
 flights2 <- flights_portland %>%
   filter(dep_delay > 15) %>%
   filter(origin == "EWR")
 nrow(flights2)     # 168 flights
 #finding percentage
 perc1 <- round(nrow(flights2) / nrow(flights_portland) * 100,2)
 perc1
 
 flights3 <- flights_portland %>%
   filter(dep_delay > 15) %>%
   filter(origin == "JFK")
 nrow(flights3)    # 193 flights
 #finding percentage
 perc2 <- round(nrow(flights3) / nrow(flights_portland) * 100,2)
 perc2
```
